...
{
    public class TarefasClienteFuncionarioRadioOrdenaController : Controller
    {
        private victor_DbGesTarefas db = new victor_DbGesTarefas();

        // GET: TarefasPorClienteEFuncionario
        public ActionResult Index(int? ClienteID, int? FuncionarioID, string ordena)
        {
            ...
            if (!string.IsNullOrEmpty(ordena) && ordena == "Desc")
            {
                ViewBag.RADIO_ASC = false;
                tarefas = tarefas.OrderByDescending(t => t.ID); ;
            }
            else
            {
                ViewBag.RADIO_ASC = true;
                tarefas = tarefas.OrderBy(t => t.ID);
            }
